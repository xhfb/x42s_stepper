...
class Code(ExtendedIntEnum):
    ...
    FAST_POSITION_CFG = 0xF1  # 快速位置设参 (Emm/X, V2.0.0+)
    FAST_POSITION_PULSE = 0xFC  # 快速位置发脉冲/角度 (Emm/X, V2.0.0+)
    ...
    GET_BATTERY_VOLTAGE = 0x38  # 读取电池电压(部分机型; X42S 常无电池)
...
class FirmwareType(ExtendedIntEnum):
    """固件类型 (X42S: X / Emm / Emm狂暴)."""

    X_FIRMWARE = 0  # X固件
    EMM_FIRMWARE = 1  # Emm固件
    EMM_TURBO = 2  # Emm 狂暴模式 (需校准; 配置布局同 Emm)
